...
class X_CIFAR(torch.utils.data.Dataset):
    def __init__(self, data_samples, size=(32, 32), dataset='CIFAR10', number_samples=60000,  increment_dif=False, FF_rep=False):
...
class X_STL(torch.utils.data.Dataset):
    def __init__(self, data_samples, size=(96, 96), number_samples=60000,  increment_dif=False, FF_rep=False):
...
class X_MNIST(torch.utils.data.Dataset):
    ...
    def resize_images(self, digit1, digit2):
        ...
        # Resize images to the same size
        digit1 = cv2.resize(digit1, self.size)  # (self.new_width, self.new_height), interpolation=cv2.INTER_AREA)
        digit2 = cv2.resize(digit2, self.size)
...
class Y_MNIST(torch.utils.data.Dataset):
    def __init__(self, data_samples, size=(28, 28), number_samples=60000, dataset='MNIST',  increment_dif=False, FF_rep=False):
    ...
    def resize_images(self, digit):

        # Resize images to the same size
        digit = cv2.resize(digit, self.size, interpolation=cv2.INTER_AREA)
    ...
    def generate_data(self):
        ...
        for i in range(self.number_samples):
            ...
            digit1 = self.resize_images(np.array(self.data_samples[idx1][0]))
